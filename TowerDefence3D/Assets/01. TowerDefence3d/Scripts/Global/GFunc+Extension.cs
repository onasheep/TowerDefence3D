...
public static partial class GFunc
{
    ...
    //! 딕셔너리에 MonsterData 프리팹을 캐싱하는 함수
    public static void AddObjs(
        this Dictionary<string, MonsterData> dict_, MonsterData[] prefabs_)
    {
        foreach (var prefab_ in prefabs_)
        {
            dict_.Add(prefab_.name, prefab_);
    ...
    //! 오버라이드
    public static bool IsInRange(this float targetValue, float minInclude, float maxExclude)
    {
        return (minInclude <= targetValue && targetValue < maxExclude);
    }
    ...
    //    {
    //        if (node_.Tile == default || node_.Tile == null) { continue; }
    //        else if (compareNode.Tile == default ||
    //            compareNode.Tile == null) { continue; }

    //        if (node_.Tile.TileIdx1D.Equals(
    //            compareNode.Tile.TileIdx1D))
    //        {
    //            resultNode = node_;
    //        }
    //        else { continue; }
